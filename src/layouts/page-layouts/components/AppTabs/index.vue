...
<template>
  <div v-if="!appConfig.hideTabs" class="main-container-tabs">
    <div class="tabs-container">
      <el-tabs v-model="editableTabsValue" type="card" class="tabs-view" :closable="multiTabs.length > 1"
    ...
    <transition name="el-zoom-in-top">
      <ul v-show="visible" class="right-view" :style="rightViewStyle">
        <li v-for="(item, key) in rightClickTags" :key="key" class="right-view-item cursor"
          :class="{ disabled: item.disabled }" @click="rightViewChange(item)">
          <span>{{ item.text }}</span>
        </li>
      </ul>
    </transition>
    <div v-if="!appConfig.hideTabsConfig" class="right-button">
      <ul>
        ...
        <li>
          <ElDropdown ref="el-dropdown-ref" trigger="click" placement="bottom-end"
            @visible-change="(e: boolean) => e && contextmenu(route)">
            <SvgIcon class="action-item cursor" name="iEL-arrow-down" />
            <template #dropdown>
              <el-dropdown-menu>
                <el-dropdown-item v-for="(item, key) in rightClickTags" :key="key" :command="{ key, item }"
                  :disabled="item.disabled" @click="rightViewChange(item)">
                  {{ item.text }}
                </el-dropdown-item>
              </el-dropdown-menu>
            </template>
